...
describe('Test Mega', function () {
  ...
  it.skip('should process login and get sid', function () {
    ...
    expect(res.master.length).to.be.equal(4);
    expect(res.key.length).to.be.equal(58);
    expect(res.privkey.length).to.be.equal(8);
  });

  it.skip('should make request', function (done) {
    var m = new Mega(process.env.USERNAME, process.env.USER_PASSWORD);
    m.login(function (err, a) {
      if (!err) {
        done();
      }
    });
